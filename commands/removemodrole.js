...
exports.run = async(client, message, args) => { // eslint-disable-line no-unused-vars
    ...
    let role = message.mentions.roles.first() || message.guild.roles.get(args[0]) || message.guild.roles.find(x => { return x.name == args.join(" ") });
    ...
    if (role != undefined || role != null) {
        ...
        let rolestrings = rolelist.map(x => { return `${message.guild.roles.get(x).name}` }).join("\n");
